...
public class GameController {
    ...
    public void endDay() throws BadStatusException {
        for(Player i:this.players){
            if(i.getEnergy()>=3){
                i.setEnergy(i.getEnergy()-3);
            }
            else{
                int n = 3-i.getEnergy();
                i.setEnergy(0);
                if(n>i.getStatus().getHp()){
                    i.setStatus(new Status(0,i.getStatus().getDurability(),i.getStatus().getAttack(),i.getStatus().getMagic()));
                }
                else{
                    i.setStatus(new Status(i.getStatus().getHp()-n,i.getStatus().getDurability(),i.getStatus().getAttack(),i.getStatus().getMagic()));
                }
            }
        }
    }
